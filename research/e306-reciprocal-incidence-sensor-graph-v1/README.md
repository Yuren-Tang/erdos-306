# Reciprocal-Incidence Sensor-Graph Theorem Packet

## Status and control

This directory is the durable repository embodiment authorized by
`#issuecomment-5143441455` under role

`E306-RECIPROCAL-INCIDENCE-SENSOR-GRAPH-THEOREM-PACKET-01`.

It is based on

`main@5a6207a493067f02c67b1cfa69fa8c5498f2c005`

and records a theorem packet only. It does not modify or supersede the accepted
fixed-ratio one-anchor manuscript, its branch, or PR `#9`.

The governing source hierarchy is:

1. the audited literal thin-anchor theorem return
   `#issuecomment-5142423848`;
2. its independent audit `#issuecomment-5142616056`;
3. the accepting Director disposition `#issuecomment-5142739676`;
4. the subsequent sensor/consequence authorizations and dispositions
   `#issuecomment-5142864059` and `#issuecomment-5142986526`;
5. the accepted consequence and phase-diagram return
   `#issuecomment-5143212284`;
6. the accepted literature audit `#issuecomment-5143349827`;
7. the accepted one-anchor proof at
   `proof-development/e306-one-anchor-v1@07ed8dcaff334ba9da4b9c87337455c2eca25002`;
...
The algebraic shell belongs to a known graphical/Fourier framework. The new
mathematical content is the arithmetic combination of heterogeneous prime
moduli, reciprocal endpoint coefficients, one shared three-point current per
edge, source-uniform inverse-residue concentration, and nested dense-core
elimination.

In particular:

- this is not an ordinary oriented cycle code;
...
- the equality endpoint `Gamma=3/2` is not included;
- no quantitative statement is uniform as the edge activity tends to zero.
