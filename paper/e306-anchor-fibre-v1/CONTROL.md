...
- **arXiv-v1 state:** mathematically, editorially, and whole-plane ready; exact source frozen and clean-built, exact finished article read cover to cover, and current portfolio mathematics reconciled through the canonical knowledge plane. The Owner must perform the arXiv submission/endorsement interaction if required.
...
`ΟΙΣ` is deliberate under classical relative attraction. The current STIX Medium setting is retained for v1. A future alternative-face comparison is permissible only if an actually superior rendered result is demonstrated; it is not a known defect in the present page.
...
## Pre-arXiv finished-object and whole-plane gates

The Owner requires two additional gates before v1: this Director must personally read the exact finished article as a whole, and must hold the current repository-wide mathematical architecture in view rather than treating the paper in isolation.

Both gates are now closed.

### Exact finished article

The Director read exact source `d21c612...` continuously from `main.tex` through all ten sections and `references.bib`, and separately inspected all 36 rendered PDF pages. No new proof gap, statement/proof mismatch, version-splice defect, clipping/collision, or other known manuscript defect was found. This is internal author/Director assurance and does not replace the independent review above.

### Current mathematical architecture

The complete authored canonical mathematical plane had already been read at `research-workbench@3c9410025628ffc1b6184be5a89afd123946488b`. Canonical `knowledge/math/**` did not change before the present sparse-target integration; the Director therefore re-read the surviving post-watermark mathematical delta, including the current E306 chain and the live BODG/GER and AffineCDC frontier, rather than pretending an unchanged corpus needed an identical second traversal.

That pass found one substantive E306 knowledge-plane drift: the canonical quantitative chapter and watermark still reflected the older full-target-row `gamma>2` epoch. The drift was repaired without conflating theorem identities. The sparse-sensor `gamma>1` local-profile and multiplicity statements are now distinct established canonical units; the older full-target-row `gamma>2` candidates remain at their own supported scope; moving-target assurance gained the current-family path without changing theorem identity.

The integration was squash-merged to `research-workbench/main` through PR #256 at

```text
3a42859dc15b8d3db43288cee4482d69f21fd6b8.
```

The post-merge exact-main slow verification run

```text
31329335817
```

passed the complete fast contract, all twelve retained computation/verifier replays, `generate_release_manifest.py --check`, and final `git diff --exit-code`. The final gate record is `research-workbench/intake/2026-08-09-e306-pre-arxiv-cover-to-cover-and-whole-plane-gate.md`; the subsequent gate-record-only update on research-workbench main changes no mathematical or generated surface.

No current neighbouring project supplies a mature theorem-level provider that should replace or enlarge the E306 article before v1. New mathematical strengthenings remain future theorems, not known defects in the fixed paper.
...
- Do **not** upload font binaries: all required ETbb and STIX Two files are available in the configured TeX environment and are referenced by filename.
...
[E306 CANONICAL KNOWLEDGE DRIFT REPAIRED / PR #256 MERGED]
[EXACT-MERGED-MAIN SLOW KNOWLEDGE GATE PASS — ACTION 31329335817]
[ARXIV-V1 SOURCE d21c612be5c3534b3b2b40361c4a861f40969565]
[CLEAN MANUSCRIPT BUILD PASS — ACTION 31326307200 / RUN 213]
