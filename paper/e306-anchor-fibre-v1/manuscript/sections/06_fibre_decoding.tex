...
\begin{proof}
Inversion permutes the nonzero residue classes modulo $r$.  The interval $[\eta Z,Z)$ contains at most
\[
 \mu_r\leq1+\frac{(1-\eta)Z}{r}\leq\frac{Z}{r}
\]
integers in one residue class modulo $r$, because $r<\eta Z$ for $r\in P\setminus B$.  With
\[
 M=|B|=\bigl(1-\eta+o_\eta(1)\bigr)\frac{Z}{\log Z},
\]
\cref{lem:cyclic-energy} gives
\[
 D_r\gg_\eta\frac{M^3}{\mu_r^2r^2}
 \gg_\eta\frac{Z^3/\log^3Z}{Z^2}
 =\frac{Z}{\log^3Z}.
\]
Moreover $M/\mu_r\gg_\eta r/\log Z\geq X/\log Z$, so the largeness hypothesis is uniform in $r$.
\end{proof}
...
There is
\[
 c_{\rm row}=c_{\rm row}(t,\gamma,\eta,b)>0
\]
such that
\[
 \delta_r:=\sum_{x\neq x_r^*}A_r(x)
...
\]
Apply \cref{eq:bernoulli-modulus} and sum over the at most $r$ nondecoder residues.  One may take $c_{\rm row}=\kappa_{\rm mod}/4$.
\end{proof}
...
This argument is unnormalized and remains valid when a decoder weight $\alpha_r$ is zero; no decoder weight is divided out, and the raw number of anchor assignments does not occur.  The lower--lower factors remain in $H_{a_B}$ and will supply the minor-frequency coercivity.

\subsection{Identification ranges and decoded frequencies}
...
\begin{proof}
The upper bound in \cref{eq:candidate-row-energy} increases with $|m|$ and decreases with $r$, so the worst permitted endpoint is $r=X$ and $|m|=M_{\mathrm{dec}}$.  There the candidate energy is $O_\eta(Z/\log^5Z)$, whereas \cref{prop:lower-row-distance} gives $D_r\gg_\eta Z/\log^3Z$.  Their ratio is $O_\eta((\log Z)^{-2})$, and is therefore below $1/8$ for all sufficiently large $X$, uniformly for every permitted $r$ and $m$.
\end{proof}

Fix once and for all a sufficiently small constant
\[
 \kappa_0=\kappa_0(t,\gamma,\eta,b)>0,
...
\begin{proof}
By \cref{eq:candidate-row-energy}, the candidate energy is $O_{b,\eta}(m^2/(Z\log Z))$, while \cref{prop:target-row-distance} is $\gg_{b,\eta}Z/\log Z$.  Since $|m|\leq T_0\leq\kappa_0Z$, choosing $\kappa_0$ sufficiently small in terms of $(t,\gamma,\eta,b)$ makes the comparison strict, uniformly over the finite set $S_b$.
\end{proof}

For every coherent anchor assignment with label $m$, define its \emph{decoded CRT frequency}
\begin{equation}
\label{eq:h-m-definition}
 h_m\in\mathbb Z/L\mathbb Z
\end{equation}
to be the unique residue class whose coordinate modulo each $q\in B$ is $m\pmod q$ and whose coordinate modulo each $r\in R_{\mathrm{rows}}$ is the chosen decoder $x_r^*$.  Thus $m$ is an ordinary integer label, whereas $h_m$ is the CRT frequency retained after fibre compression.

Because $b$ is squarefree, agreement modulo every $r\in S_b$ is agreement modulo $b$.  Hence, when $|m|\leq T_0$, every prime and target coordinate of $h_m$ agrees with $m$, so
\begin{equation}
\label{eq:h-m-full-identification}
 h_m=m\pmod L.
\end{equation}
In the transition range $T_0<|m|\leq M_{\mathrm{dec}}$, only the coordinates indexed by $P$ are asserted to agree with $m$; the target coordinates in $S_b$ may differ.  The estimates in that range use only the prime-pair factors and therefore require no stronger identification.
