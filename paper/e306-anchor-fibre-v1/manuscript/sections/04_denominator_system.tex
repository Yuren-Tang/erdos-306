...
\end{equation}
We assume the exact admissible inequalities for the present centred no-wrap construction
\begin{equation}
...
\begin{proof}
Apply the prime number theorem at $uy$ and $vy$.  Replacing $\log(uy)$ and $\log(vy)$ by $\log y$ introduces only a relative $o(1)$.
\end{proof}
...
\begin{proof}
Combine \cref{eq:theta-limit,eq:complete-square-load,eq:sigma-E}.  Both summands are retained, so the statement is valid without first deciding which variance contribution dominates.
\end{proof}
