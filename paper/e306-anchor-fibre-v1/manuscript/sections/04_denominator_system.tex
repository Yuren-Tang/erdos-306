...
\[
 t=\frac ab\in(0,1)
\]
with squarefree $b$, a finite forbidden set $\mathcal T$, and a fixed exponent $\gamma>1$.  Put
\begin{equation}
\label{eq:lambda-gamma}
 \lambda_\gamma=\frac{(\log\gamma)^2}{2}.
\end{equation}
We assume the sharp admissible inequalities
\begin{equation}
...
\end{equation}
All asymptotic statements are for $X\to\infty$ with $t$, $\gamma$, $b$ and $\mathcal T$ fixed.  Unless stated otherwise, implied constants may depend on the fixed tuple $(t,\gamma,b)$ and on fixed numerical choices, but not on $X$ or on the later Gaussian cutoff $C$.
...
We use the following standard consequence of the prime number theorem; see Montgomery and Vaughan~\cite[Chapter~1]{montgomery-vaughan-2007}.
...
\begin{proof}
Apply the prime number theorem at $uy$ and $vy$ in its tail-uniform form.  Replacing $\log(uy)$ and $\log(vy)$ by $\log y$ introduces only a relative $o(1)$.
\end{proof}
...
\[
 ...
 \sim\frac1{X\log X};
\]
the upper endpoint is negligible because $\gamma>1$ is fixed.  The cardinality statements follow from \cref{lem:fixed-ratio-prime-supply}.  Finally, partial summation on $[Z/2,Z)$ gives the two estimates in \cref{eq:B-power-sums}; explicitly,
\[
 \int_{Z/2}^{Z}\frac{du}{u^2\log u}
 \sim\frac1{Z\log Z}.
\]
\end{proof}
...
\label{eq:family-size-modulus}
 ...
 \log L=(1+o(1))Z=o(M_X).
\end{equation}
The target rows have only $O_b(Z/\log Z)$ elements and do not affect the leading family size.  Every denominator is at most $Z^2$ for large $X$.

\subsection{Exact centring and the variance phase boundary}
...
\label{eq:theta-definition}
 \theta=\frac{t}{\Lambda}.
\end{equation}
By \cref{prop:arithmetic-capacity}, $\theta$ lies in a fixed compact interval $I_{t,\gamma}\subset(0,1)$ and
\begin{equation}
...
\begin{proof}
Combine \cref{eq:theta-limit,eq:complete-square-load,eq:sigma-E}.  Both summands are retained, so the statement is valid without first deciding which variance provider dominates.
\end{proof}
...
\label{eq:variance-gamma-two}
 \sigma_{\mathcal E}^2
 \sim\frac{\alpha(1-\alpha)\tau(b)}{Z\log Z}.
\end{equation}
...
\begin{proof}
The ratio of the target-row summand in \cref{eq:total-variance} to the pair summand is asymptotic, up to a fixed positive factor, to
\[
 \frac{X^2\log^2X}{Z\log Z}
 \asymp X^{2-\gamma}\log X.
\]
It tends to $0$ for $\gamma>2$, to infinity logarithmically for $\gamma=2$, and to infinity polynomially for $\gamma<2$.
\end{proof}

Thus $\gamma=2$ remains a genuine variance boundary, but it no longer requires a separate proof architecture: every later estimate is stated first with the total variance \cref{eq:total-variance}.
...
\end{equation}
Compactness of $I_{t,\gamma}$ gives a constant
\[
 \kappa_{\rm mod}=\kappa_{\rm mod}(t,\gamma)>0
\]
such that
\begin{equation}
...
\end{equation}
For compatibility with the notation in the unchanged anchor-block estimates, write $\kappa_b:=\kappa_{\rm mod}$ there; this is the same Bernoulli modulus constant and is not the decoder cutoff introduced later.  Consequently,
\begin{equation}
