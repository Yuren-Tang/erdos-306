...
\end{equation}
All asymptotic statements are for $X\to\infty$ with $t$, $\gamma$, $b$ and $\mathcal T$ fixed.
...
\[
 \Lambda_b
 =\left(\sum_{r\mid b}\frac1r\right)V_B
 =O_b(1/\log Z)=o_b(1),
\]
...
\[
 \sum_{r\mid b}\sum_{q\in B}\frac1{r^2q^2}
 =\tau_bW_B
 \sim\frac{\tau_b}{Z\log Z}.
\]
...
\begin{proposition}[Complete variance in the three regimes]
...
\label{eq:variance-gamma-two}
 \sigma_{\mathcal E}^2
 \sim\frac{\alpha(1-\alpha)\tau_b}{Z\log Z}.
\end{equation}
...
\begin{proof}
Compare the two summands in \cref{eq:complete-square-load}.  Their ratio, target over pair, is asymptotic to a constant multiple of
\[
 \frac{X^2\log^2X}{Z\log Z}
 \asymp X^{2-\gamma}\log X.
\]
It tends to $0$ for $\gamma>2$, to infinity logarithmically for $\gamma=2$, and to infinity polynomially for $\gamma<2$.  Now use \cref{eq:theta-limit}.
\end{proof}

This separate treatment of $\gamma=2$ is essential.  The coefficient scale, the major range, and the normalized minor totals all use the actual variance in \cref{prop:variance-regimes}.
...
\end{equation}
Compactness of $I_{t,\gamma}$ gives a constant $\kappa_b>0$---allowed to depend on the fixed $t$ and $\gamma$---such that
\begin{equation}
\label{eq:bernoulli-modulus}
 |(1-\theta)+\theta\exp(2\pi i u)|
 \leq\exp(-\kappa_b\dist{u}^2).
\end{equation}
Consequently,
\begin{equation}
