...
Thus the visible arithmetic boundary is the squarefree-denominator condition, strengthened by finite deletion.  The fixed-target theorem also yields finite prescription, disjoint realization, common refinements, and countably infinite proper refinement chains.  Prime dilution then gives the converse direction in \cref{thm:main} without numerator induction or special reduced-denominator cases.
...
\]
The exact admissible range for the present centred no-wrap construction is
\begin{equation}
...
\[
 ...
 \right).
\]
There remains one genuine phase boundary: prime pairs supply the leading variance when $\gamma>2$, whereas the target rows supply it at and below $\gamma=2$.  This change in the dominant contribution does not split the proof architecture.  A universal full-coordinate cutoff
\[
...
The archived Lean~4 version~0.0.3 formalization machine-checks the headline existence theorem~\cite{tang-erdos-306-formal}.  The present one-anchor argument is a later conceptual reorganization and is not formalized line by line; in particular, the parameterized fixed-target theorem, the Gaussian coefficient asymptotic, and the entropy, Hamming, and refinement consequences are not claimed to be covered by that formalization.
