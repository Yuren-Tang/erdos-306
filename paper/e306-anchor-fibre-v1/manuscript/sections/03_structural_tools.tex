...
This section contains the general deterministic results used by the arithmetic construction.  The complete-family estimate controls capacity.  The product-fibre theorem performs the load-bearing compression.  The syndrome theorem detects hidden target coordinates.  The final proposition assembles a positive skeleton contribution against finitely many error lanes.  None of these statements supplies the prime-specific rigidity, row distance or minor-arc estimates proved later.
...
This is an anchor-dependent weighted average.  No sequential conditional kernel is asserted.  In the E306 application we use the unnormalized theorem, so zero decoder weights never create a normalization issue.
