...
We isolate four deterministic principles used by the arithmetic construction.  The first quantifies the loss caused by collisions in complete weighted $k$-uniform families.  The second compresses a product fibre to one decoded tuple without discarding unassigned factors.  The third gives an intrinsic finite-group criterion for target observability.  The fourth compares a positive decoded skeleton with its minor lanes.  The statements are independent of the semiprime specialization; the concrete proof supplies their arithmetic hypotheses in later sections.
...
The semiprime family uses the exact pair identity.  The higher-uniformity theorem is retained because it gives genuinely stronger information: it identifies, for fixed or moving $k$, the precise collision parameter detected by the first two power sums.  It does not by itself supply the rigidity or decoding theory required in a higher-uniformity arithmetic application.
...
\subsection{Syndrome separation for finite target groups}
...
The theorem separates qualitative observability from quantitative decoding.  For one finite system, $\delta>0$ is precisely injectivity; for a sequence of systems, an explicit lower bound for $\delta$ supplies the scale.  The result is an absolute-value estimate, not a classification of all possible cancellation arguments, and an application must still identify the intended decoder by comparing its energy with competing syndromes.  In the E306 construction the target rows furnish the concrete sensors, while the prime rows are controlled by the stronger arithmetic distance estimates of \cref{sec:fibre-decoding}.  If $H$ is trivial, no target-coordinate condition remains and the minimum over nonzero elements is omitted.

\subsection{Decoded-skeleton positivity}

Retain the notation of \cref{thm:weighted-compression}.  Partition the anchor-assignment set into one major set and finitely many minor lanes,
\[
