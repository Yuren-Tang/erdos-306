...
\[
 N<|m|\leq T_0,
\]
the decoded tuple is the genuine integer $m$ modulo $L$.  The constant $\kappa_0$ in \cref{eq:T-full} was chosen so that $|m|/e\leq1/4$ for every $e\in\mathcal E$.  Hence
\begin{equation}
...
The proof uses the total actual-family variance in both the major and the first minor range.  The leading variance provider still changes at $\gamma=2$, but the following prime-only transition sector and the remaining estimates are now part of one six-sector architecture in every regime.
