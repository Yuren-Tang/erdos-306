...
Fix squarefree $b\ge3$. For each $X$, keep the direct family $\mathcal E=\mathcal E_X$, the common denominator $L=L_X$, and the Bernoulli parameter $\theta=1/(b\Lambda)$. For a lattice target $t\in L^{-1}\Z$, write
\[
W_X(t)=
\Pr\left(\sum_{e\in\mathcal E}\frac{\xi_e}{e}=t\right).
\tag{8.1}
\]
...
\]
Comparing with the central target $t_0=1/b$ gives the exact identity
\[
F_j(h)=e(-hj/L)F_0(h).
\tag{8.4}\label{eq:unit-character}
\]
Hence
\[
|F_j(h)|=|F_0(h)|
\tag{8.5}
\]
for every $h$ and $j$. All anchor, row, decoder, fibre, and coherent-tail estimates in Sections \ref{sec:anchor}--\ref{sec:gaussian} are therefore unchanged.
...
\tag{8.7}
\]
On a genuine integer frequency $m$, equations \eqref{6.10} and \eqref{eq:unit-character} give, uniformly for $|m|\le C/\sigma$,
\[
F_j(m)
=
\exp\left(
...
For fixed $C,U>0$, uniformly for integers $j$ satisfying \eqref{8.6},
\[
\sigma
...
\begin{proof}
The Taylor remainder is the same uniform error as in \eqref{6.10}. For $|u|\le U$, the functions
\[
...
By \eqref{8.5}, every modulus estimate in Sections 6.2--6.5 is unchanged. If $\mathfrak R_{X,C}(j)$ is the complete contribution outside the genuine integer range $|m|\le C/\sigma$, then
\[
\limsup_{X\to\infty}
\sup_{|j|\le UL\sigma}
...
Combining Proposition \ref{prop:moving-major} with \eqref{8.12}, first taking $X\to\infty$ with $C,U$ fixed and then $C\to\infty$, gives
\[
...
\tag{8.13}
\]
uniformly for \eqref{8.6}.

\subsection{No-wrap and simultaneous exact realization}

The limiting load satisfies
\[
\Lambda\longrightarrow\frac{(\log3)^2}{2}>\frac13\ge\frac1b.
\]
Thus $1/b$ lies a fixed positive distance from the endpoints of the eventual interval $[0,\Lambda]$. Since $\sigma\to0$, for each fixed $U$ all targets $t_j$ with \eqref{8.6} lie in $[0,\Lambda]$ for sufficiently large $X$. Lemma \ref{lem:no-wrap} therefore gives
\[
W_X^{\mathrm{quot}}(t_j)=W_X(t_j).
\tag{8.14}
\]
Together with \eqref{8.13}, this proves
\[
L\sigma
W_X\!\left(\frac1b+\frac{j}{L}\right)
=
\Phi\!\left(\frac{j}{L\sigma}\right)+o_U(1),
\tag{8.15}\label{eq:moving-profile}
\]
...
\tag{8.17}
\]
By \eqref{2.19} and Lemma \ref{lem:family-size},
\[
\sigma\to0,
\qquad
L\sigma\to\infty.
...
Dividing \eqref{eq:moving-profile} by the central local limit also gives
\[
\frac{W_X(1/b+j/L)}{W_X(1/b)}
=
