...
Retain a fixed reduced target $t=a/b\in(0,1)$ with squarefree $b$, a fixed anchor ratio $\eta\in(0,1)$, a fixed admissible exponent $\gamma$, and a fixed forbidden set.  Write $\mathcal E_X$, $L_X$, $\Lambda_X$, $\theta_X$ and $\sigma_X$ for the parameterized objects above.  Let
\[
 M_X=|\mathcal E_X|,
 \qquad
...
\label{eq:quantitative-coefficient}
 L_XP_X(t)\geq\frac{c_{t,\gamma,\eta}}{\sigma_X}
\end{equation}
for a fixed $c_{t,\gamma,\eta}>0$.
\end{corollary}
...
For fixed $t$, $\gamma$, $\eta$ and finite forbidden set,
\begin{equation}
...
\label{eq:sharp-target-coefficient-regimes}
 ...
 \displaystyle
 \frac{\sqrt{Z\log Z}}
 {\sqrt{2\pi\alpha(1-\alpha)(\eta^{-1}-1)\tau(b)}},&1<\gamma\leq2.
 \end{cases}
\end{equation}
\end{theorem}
...
\[
 \sigma_X=O_{b,\eta}\!\left(\frac1{X\log X}+\frac1{\sqrt{Z\log Z}}\right)=o(1),
\]
...
\[
 |\log\sigma_X|=O_{b,\eta}(\log Z)=o(M_X).
\]
...
There are constants $\rho,\zeta>0$ and, for all sufficiently large $X$, a family $\mathcal F_X\subseteq\mathcal R_X(t)$ such that
\[
 |\mathcal F_X|\geq\exp(\zeta M_X),
 \qquad
...
There are constants $c,\zeta'>0$ such that, for all sufficiently large $X$, there are at least $\exp(\zeta'M_X)$ distinct ordered pairs $(\mathcal P,\mathcal N)$ of disjoint subsets of $\mathcal E_X$ satisfying
\[
