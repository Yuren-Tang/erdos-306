...
For each $X$, let $c_X(u)\geq0$ be an exact normalized coefficient, indexed by $u$ in a finite set $K_X\subset K$, and let $L_X>0$ and $\sigma_X\to\infty$.  Assume the following data.
...
\]
exactly.  Insert LOCAL and TAIL, first fixing $C$ and then letting $X\to\infty$.  Since $K$ is compact, $\inf_{u\in K}\Phi(u)>0$.  Letting $C\to\infty$ gives \cref{eq:one-well-composition-output} uniformly and therefore positivity.  REAL converts positivity into a quotient witness; the final ambient equality uses only the separately stated lifting datum.  The order is thus $C\to X$ and only afterwards $C\to\infty$, and no local phase is taken from the positive tail majorant.
\end{proof}
