...
The argument has three stages. Reciprocal dispersion first supplies a forcing floor; below that floor every anchor coordinate is synchronized to one exact integer label, while above it fingerprint counting gives a small weighted noncoherent tail. We prove these statements for a fixed-ratio prime block and then apply them to the anchor block $B$.
...
\]
The proof of \cref{lem:reciprocal-dispersion}, applied with $\mathcal F=C_m$, shows that at least half of the left sides exceed a constant times $|C_m|/Y$. The size hypothesis makes $|m|/(pq)$ at most half that amount. Thus these $p$ contribute $\gg_\delta |C_m|^3/Y^2$ for each admissible $q$, and summing over $q$ proves the lemma.
\end{proof}

\subsection{Low energy forces one exact label}

We first show that energy below the forcing scale produces one dominant class, and then remove the remaining exceptional coordinates.
...
\]
Since every other substantial class has at least $s_0$ elements,
\[
 \sum_{j\neq i}(n_j-1)\geq\frac12(S-n_i).
\]
If some class has size at least $S/2$, then its complement has size at least $\rho N/2$, and its term is $\gg_\rho N^4$. Otherwise $S-n_i\geq S/2$ for every $i$, while power mean gives
\[
 \sum_i n_i^3\geq\frac{S^3}{M_0^2}.
\]
In either case,
\[
...
\label{prop:exact-anchor-rigidity}
...
\]
This proves the label bound. Together with the low-energy hypothesis, $\sigma_Q\asymp_\delta1/(Y\log Y)$ and $|C|\asymp_{\delta,\rho}Y/\log Y$, it gives
\[
 |m|=O_{\delta,\rho,c_w}\!\left(\frac{Y^{3/2}}{\sqrt{\log Y}}\right)=o_\delta(|C|Y).
\]
If $q\notin C$, put $d\equiv a_q-m\pmod q$. The preceding estimate absorbs the term $|m|/(pq)$, so, as in the proof of \cref{lem:cross-label-energy}, reciprocal dispersion against $p\in C$ forces the edges incident with $q$ to contribute $\gg_\delta |C|^3/Y^2$. Distinct exceptional $q$ give disjoint edge sets, whence
\[
...
Below the forcing floor the preceding propositions give exact coherent labels. At and above that floor we instead count assignments through a small fixed fingerprint and use the resulting count only after weighting by the anchor modulus.
...
\begin{equation}
\label{eq:anchor-exact-energy}
 Q_B(a_B)=m^2\sigma_{B,0}^2.
\end{equation}
We call precisely these below-floor assignments \emph{coherent}; assignments with $Q_B\geq F_B$ are \emph{noncoherent} and will be controlled only through their total weighted mass.
...
\begin{equation}
\label{eq:noncoherent-anchor-tail}
...
\label{prop:anchor-partition}
The complete anchor weight satisfies
\begin{equation}
\label{eq:anchor-weight-total}
 P_{\mathrm{anc}}:=\sum_{a_B}T_B(a_B)\ll_\eta Z\log Z,
\end{equation}
while its noncoherent part satisfies \cref{eq:noncoherent-anchor-tail}. Every coherent anchor assignment carries a unique exact integer label and obeys \cref{eq:anchor-exact-energy}.
\end{proposition}
