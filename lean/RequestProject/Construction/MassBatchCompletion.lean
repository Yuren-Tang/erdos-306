...
`Q` with `MassBatchSupply (D.withQ Q)` exists.  The load threshold `k1` is supplied
...
lemma exists_massBatch_of_base_load {T : Finset ℕ} {b : ℕ} (hb : 3 ≤ b)
    ...
    (hload : ∀ k0 : ℕ, k1 ≤ k0 →
      (1 : ℝ) / 2 ≤ ∑ pq ∈ (blockPrimes k0).offDiag.filter (fun pq : ℕ × ℕ => pq.1 < pq.2),
        (1 : ℝ) / ((pq.1 : ℝ) * (pq.2 : ℝ))) :
    ∃ Q : Finset ℕ, MassBatchSupply (D.withQ Q) :=
...
lemma exists_massBatch_for_gadget_set {T : Finset ℕ} {b : ℕ} (hb : 3 ≤ b)
    ...
    (hctrl : ConstructionData.recipLoad (ctrlEdges BS) ≤ 3 / (4 * (b : ℝ)))
    (hsum : 3 / (4 * (b : ℝ)) +
    ...
    ∃ Q : Finset ℕ,
      MassBatchSupply ((⟨BS, ∅, b.primeFactors, S⟩ : ConstructionData T b).withQ Q) := by
  set D0 : ConstructionData T b := ⟨BS, ∅, b.primeFactors, S⟩ with hD0
  have hRprime : ∀ r ∈ D0.R, Nat.Prime r := fun r hr => Nat.prime_of_mem_primeFactors hr
  have hRout' : ∀ r ∈ D0.R, r ∉ blockSupport D0.BS := hRout
  have B0 : BaseLoadBudget D0 :=
  ...
  have hdisj := ctrlEdges_disjoint_gadgetEdges_of_denominator_primes_outside_blockSupport D0 hRprime hRout'
