...
If Theorem 2 holds for a fixed squarefree `b`, then for every integer `a>=1` and every finite `T` there is an avoiding representation of `a/b`.

**Proof.** Induct on `a`.  Having represented `a/b` by a finite set `A`, apply Theorem 2 with forbidden set `T union A`.  The new unit representation is disjoint from the old one, and their reciprocal sums add. ∎

### Lemma 4 — denominators `1` and `2`

It is enough to prove Theorem 2 for squarefree `b>=3`.

**Proof.** Use successive avoiding representations and
...
All denominators on the right are squarefree and at least `3`; avoidance preserves distinctness between the component representations. ∎
...
Choose `X` so large that every prime in `P` exceeds every prime divisor of `b`, and so that every product used below exceeds every element of `T`; this last condition is vacuous if `T` is empty.
...
For every fixed `1<u<v`, uniformly for `y` tending to infinity,

```text
# {p prime : uy <= p < vy} = (v-u+o(1))y/log y.
```

In particular, for all sufficiently large `y`, the count is between positive constant multiples of `y/log y`.

**Proof.** Write the PNT as `pi(t)=t/log t(1+epsilon(t))`, where
...
For the square sum, split `[X,X^3)` into dyadic intervals.  Lemma 5 gives at most `C 2^k/k` primes in `[2^k,2^(k+1))`, so the contribution is at most `C/(k2^k)`.  The resulting geometric tail is `O(1/(X log X))`.

The estimate for `M` is Lemma 5 with the interval `[Z/2,Z)`.  Since every `q in B` is at least `Z/2`,
...
```text
V_B = log log Z-log log(Z/2)+o(1/log Z)=O(1/log Z).
```
...
**Proof.** Since every `p in P` is larger than the prime divisors of `b`, unique factorization separates a product of two primes in `P` from a product `rq` with `r|b` and `q in B`.  It also proves distinctness within each family.  All factors are distinct, so the products are squarefree.  The choice of `X` gives avoidance, and every denominator plainly divides `L`.
...
This proves the asserted load interval.

For the upper square bound,

```text
sum_(p<q in P)1/(p^2q^2) <= U_X^2/2
 = O(1/(X^2 log^2 X)).
```
...
We now prove the one-block input from scratch.  Let `Y` be large, let `Q` be a set of primes in `[Y,2Y)`, and suppose

```text
c Y/log Y <= N:=|Q| <= C Y/log Y.
```
...
sigma_Q^2=sum_(p<q in Q)1/(p^2q^2) asymp 1/(Y^2 log^2 Y).
```
...
```text
sum_(p in F)||d p^(-1)/q||^2 >= c_D s^3/Y^2.
```
...
Then `vp=d mod q`.  For fixed nonzero `v`, this determines one residue class for `p mod q`; an interval of length at most `q` contains at most two integers in such a class.  There are at most `2(s/16)+1` possible `v`, so fewer than `s/2` primes are close.  At least `s/2` terms exceed `delta`, which gives the result. ∎
...
Let `C_m,C_m'` be disjoint label classes with `m!=m'`, both labels of absolute value at most `B<Y^2/4`.  Suppose
...
Lemma 8 applied to `F=C_m` says that at least half the left sides exceed `|C_m|/(64Y)`.  The size assumption gives
...
```text
mathcal Q(a)<c_w Y/log^3 Y
```

implies that `a` has a unique dominant label.

**Proof.** If `mathcal Q(a)=0`, every centred lift is zero.  Hence every coordinate is `0`, and the conclusion is immediate.  Assume henceforth that

```text
0<R:=mathcal Q(a)<c Y/log^3 Y,
```
...
with `A_rho` a sufficiently large fixed constant.  Call a pair bad if `|H_pq|>B_0`.  Since `pq<4Y^2`, each bad pair contributes more than `B_0^2/(16Y^4)`, so
...
For small enough `c` and large `Y`, `B_0<Y^2/4`.  Suppose no dominant class exists.  Call a label class small if its size is less than
...
**Case 2.** Small classes contain less than `rho N/4` vertices.  After removing the base vertex and bad neighbours, the substantial classes have total size
...
Apply Lemma 9 to ordered pairs of distinct substantial classes.  Every undirected cross edge is then counted twice, so
...
If the largest class has size at least `S/2`, then it has `>>N` vertices and at least `>>_rho N` vertices lie outside it; its single term is `>>_rho N^4`.  If every class has size less than `S/2`, then `S-n_i>=S/2`, and power mean gives
...
Both cases contradict `R<cY/log^3Y` once `c` is chosen below the resulting fixed constants.  This proves the proposition. ∎
...
```text
mathcal Q(a)>=m^2 sum_(p<q in C)1/(p^2q^2)
             >=c_rho m^2 sigma_Q^2.
```

This proves the label bound.

Let `E_0=Q\C`.  The preceding bound and the cold upper bound imply, for large `Y`,

```text
|m|<=|C|Y/128.
```

For each `q in E_0`, apply Lemma 8 to `F=C` and the nonzero difference between `a_q` and `m`.  Repeating the subtraction used in Lemma 9 shows that the cross energy from `q` into `C` is at least `c|C|^3/Y^2`.  These edge sets are disjoint as `q` varies.  Therefore
...
```text
|E_0| <= C mathcal Q(a)Y^2/N^3 < C'c_w.
```
...
After squaring and summing, Lemma 8 prevents both energies from being below `g s^3/Y^2` if `g` is small enough. ∎
...
with fixed large `A`, and use a predetermined fingerprint set `F` of size `s`.  The lower bound on `R` gives `s>=16`; the upper bound gives `s<=N/2`.
...
vertices are forced to one residue.  Choosing the exceptional vertices and their residues gives at most

```text
exp(C(s+t)logY)
```

possibilities.  Thus the logarithmic count is
...
such that every assignment with `Q_B<F_B` has a unique exact label `m`, and

```text
Q_B=m^2sigma_B0^2.
```

The equality holds because Proposition 11 bounds `|m|` by `O(Z^(3/2)/sqrt(log Z))=o(Z^2)`, so `m` is the centred lift on every top pair.

Let

```text
T_B(a_B)=product_(q<q' in B)|(1-theta)+theta e(H_qq'(a_B)/(qq'))|.
```
...
Below the floor the assignments are distinct integer labels and
...
Thus the complete top partition satisfies

```text
P_top:=sum_(a_B)T_B(a_B)<=CZlogZ,
```

while its noncoherent part is exponentially small.

All properties of the full-top auxiliary family have now been verified: distinctness, avoidance, load `O_b(1/logZ)`, square load `O_b(1/(ZlogZ))`, compactness of `theta`, no-wrap, and negligible variance contribution.
...
Fix a top assignment.  For each row `r in R_rows` and residue `x mod r`, let `u_rq(x) in R/Z` be the phase `h/(rq)` determined by the row residue `x` and the fixed top residue modulo `q`.  If the row residue changes by `d`, then CRT gives

```text
u_rq(x+d)-u_rq(x)=d q^(-1)/r mod 1.
```
...
```text
sum_j ||dc_j/r||^2 >= c M^3/(mu^2r^2).
```

**Proof.** Multiplication by `d` permutes the nonzero residues, so the multiplicity bound remains `mu`.  For any integer `t>=1`, at most `2mut` terms can have circle distance less than `t/r`.  Take `t=floor(M/(4mu))`.  For large `M/mu`, at least `M/2` terms have distance at least `M/(8mur)`.  Their squares give the claim. ∎
...
```text
D_r:=min_(d!=0 mod r)sum_(q in B)||d q^(-1)/r||^2
    >> Z/log^3Z
```
...
**Proof.** For every `q in B`, `q` is coprime to `r`.  If `d!=0 mod r`, then `dq^(-1)` is nonzero modulo `r`; hence

```text
||dq^(-1)/r||>=1/r.
```

Sum the squares over all `M` top primes. ∎

This direct proof is stronger than needed and uses no distribution theorem for primes in residue classes.

## 7. Product-fibre compression and retained skeleton

For a row `r`, define its phase energy and Bernoulli kernel by

```text
E_r(x)=sum_(q in B)||u_rq(x)||^2,
A_r(x)=product_(q in B)|(1-theta)+theta e(u_rq(x))|.
```
...
For `x!=x_r^*`, the squared distance between their phase vectors is at least `D_r`.  Coordinatewise triangle inequality and `(a+b)^2<=2a^2+2b^2` give
...
Delta:=sum_(r in R_rows)delta_r
 <= Z^2 exp(-cZ/log^3Z)=o(1).
```

Indeed there are fewer than `Z` lower prime rows, each has `r<Z`, and the finitely many `b` rows are smaller.

Now decompose the absolute Fourier product for a fixed top assignment.  Assign:

- internal top denominators `qq'` to `T_B`;
...
No denominator is omitted or assigned twice.

### Lemma 18 — exact retained-skeleton inequality

For any nonnegative row kernels satisfying
...
The final two terms are at most `exp(Delta)-1`.
...
```text
sum_(x_R)product_rA_r(x_r)=product_rsum_(x_r)A_r(x_r)
 <=product_r(A_r(x_r^*)+delta_r).
```

Subtract the decoder-point product and add back its actual value with `G(x^*)`.  Since `A_r(x_r^*)<=1`,
...
This is the required globally accumulated fibre estimate.
...
so `E_r(x)>=D_r/2-E_r(m)>3D_r/8>E_r(m)`.
...
**Proof.** The candidate energy is largest at the smallest row `r=X`.  There it is
...
Proposition 15 gives `D_r>>Z/log^3Z`.  Their ratio tends to zero uniformly. ∎
...
primes, and the complete-pair energy on the decoded skeleton satisfies
...
uniformly because `y>=X/2` tends to infinity.  This proves the prime count.  Endpoint rounding changes at most two primes and is harmless.
...
The ratio is already in `[0,1/2]`; hence its circle distance is at least `1/9`.  The number of distinct unordered pairs is

```text
binom(K_m,2)>>|m|/(log|m|)^2.
```

All these denominators are lower-lower factors retained in `G`, and all their prime coordinates decode to `m`.  Summing their squared circle distances proves the claim.  The sign of `m` is irrelevant because circle distance is even.  Overlap between intervals belonging to different labels is irrelevant: the estimate is pointwise for each Fourier frequency. ∎
...
For each fixed top assignment, Lemma 18 splits the lower-coordinate sum into the decoded skeleton point and nondecoder error.  Thus all frequencies fall into exactly five sectors:
...
5. noncoherent top assignments and every nondecoder fibre term.

This is exhaustive because `L` is squarefree and the coordinate map over `P union S_b` is a bijection.  There is no additional lift, sibling, or unassigned coordinate.  It is disjoint because the top coherent/noncoherent dichotomy, the four label intervals, and the decoder/nondecoder split are each disjoint.
...
Compactness of `I_b` gives constants `rho_b>0` and `M_b<infinity` such that, for `|z|<=rho_b`,
...
The least denominator in `E` is at least `X^2`.  Uniformly for `|m|<=N`,

```text
max_e|m/e| <<_C logX/X ->0.
```
...
Hence the logarithm is valid and the aggregate remainder tends to zero for fixed `C`.  The linear term cancels exactly:
...
For large `X`, choose the remainder small enough that

```text
Re F(m)>=a_b exp(-2pi^2m^2sigma_E^2)
```

with fixed `a_b>0`.  Conjugation pairs `m` and `-m`, so the major sum is real.  If `C>=1`, the interval `|m|<=1/(4sigma_E)` lies inside the major range for large `X`, and on it the Gaussian is bounded below by a positive constant.  Therefore
...
Hence `||m/e||=|m|/e` and the Fourier-energy bound gives

```text
|F(m)|<=exp(-c_bm^2sigma_E^2)
```

for a fixed `c_b>0`, after absorbing the compact factor between `sum e^-2` and `sigma_E^2`.
...
all prime coordinates decode to `m`.  Proposition 21 and the retained lower-lower factors give

```text
|F(h_m)|<=exp(-c|m|/(log|m|)^2).
```

Throughout this range `log|m|` is comparable with `logX`, so every term is at most

```text
exp(-cX^2/log^2X).
```

There are at most `2M_dec+1=O(X^4)` labels.  Therefore the whole sector is

```text
O(X^4exp(-cX^2/log^2X))=o(1/sigma_E).
```

No identification of the `b` coordinates is used here.
...
```text
sum_(|m|>M_dec)exp(-kappa_bm^2sigma_B0^2)
 <= C sigma_B0^(-1)
...
All constants depending on `b`, the compact interval `I_b`, and the one-block theorem are fixed first.  Choose `C` as in Sector II.  Then choose `X` above the finite maximum of the thresholds required for:

- prime counts and Abel sums;
- exclusion of the support of `b` and avoidance of `T`;
- load interval, no-wrap, and variance comparison;
...
- Taylor disk and aggregate remainder;
- the strict comparisons for Sectors III–V.

For this `X`, the sum of Sectors II–V is less than, say,
...
By Lemma 7, `A` consists of distinct squarefree semiprimes and avoids `T`.  This proves Theorem 2.  Lemmas 3 and 4 prove sufficiency in Theorem 1, and necessity was already proved. ∎

## 15. Independence and route closure

Every load-bearing implication used by the one-anchor architecture is proved above.  The only external theorem is the PNT.  In particular:
...
- the retained-skeleton inequality uses the actual Bernoulli factors after absolute values and assigns every denominator exactly once;
- the row code distance is uniform, with a separate stronger proof for every `r|b`;
- decoder identification and the use of squarefreeness are explicit;
...
The route is therefore mathematically complete at proof-development level and is ready for independent route audit.